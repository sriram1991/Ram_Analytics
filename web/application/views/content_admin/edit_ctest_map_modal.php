...
<form method="POST" action="#" name="EditCourseAssessmentMapForm" id="EditCourseAssessmentMapForm" role="form">
    <div class="row"> 
                    ...
                    <select required="" autocomplete="off" name="module_name" id="module_name" class="form-control input" size="1" tabindex="1" aria-required="true">
                        ...
                        if(isset($module_list)){
                            foreach ($module_list as $res) {
                                if($res->module_name == $ctest_map_details['module_name']){
                    ...
                    <select required="" autocomplete="off" name="group_name" id="group_name" class="form-control input" size="1" tabindex="1" aria-required="true">
                        ...
                        if(isset($group_list)){
                            foreach ($group_list as $res) {
                                if($res->group_name == $ctest_map_details['group_name']){
                    ...
                    <select required="" autocomplete="off" name="subject_name" id="subject_name" class="form-control input" size="1" tabindex="1" aria-required="true">
                        ...
                        if(isset($subject_list)){
                            foreach ($subject_list as $res) {
                                if($res->subject_name == $ctest_map_details['subject_name']){
                    ...
                    <select required="" autocomplete="off" name="resource_id" id="resource_id" class="form-control input" size="1" tabindex="2" aria-required="true">
                        ...
                        if(isset($assessment_list)){
                            foreach ($assessment_list as $ress) {
                                if($ress->test_id == $ctest_map_details['test_id']){
                                    echo "<option value='".$ress->test_id."' selected>Test ".$ress->test_no."</option>";
                                } else {
        ...
        <div class="col-xs-12 col-sm-4 col-md-4">   
            ...
            <div class="form-group control-group">
                <div class="control">
                    <div class="input-group date">
                        <input type="text" required="" value="<?php //if(isset($ctest_map_details)) echo $ctest_map_details['schedule']; ?>" autocomplete="off" name="schedule" id="schedule" class="form-control input schedule" placeholder="Select Publish Date" tabindex="3" aria-required="true"> 
            ...
            <div class="form-group control-group">
                <div class="input-group control">
                    <input type="text" required="" value="<?php if(isset($ctest_map_details)) echo $ctest_map_details['schedule']; ?>" autocomplete="off" name="schedule" id="schedule" class="form-control input edit-ctest-schedule" placeholder="Select Publish Date" tabindex="3" aria-required="true"> 
                    <span class="input-group-btn">
                        <button class="date-button btn btn-default" type="button"><i class="glyphicon glyphicon-th"></i></button>
                    </span>
                </div>
                <div id="edit-ctest-date-picker"></div>
                <span class="help-block"></span>
            </div>
        </div>
    ...
    <div class="row">
        <div class="box">
            ...
            <div class="box-body table-responsive">
                ...
                <table id="edit_assessment_browser" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>SL.NO</th>
                            <th>Test NO</th>
    ...
    <input type="hidden" id="course_test_mapid" value="<?php if(isset($ctest_map_details)) echo $ctest_map_details['map_id'];?>"></input>
</form>
...
// var edit_schedule_date = $('.schedule').pickadate({
//     format: 'yyyy-mm-dd',
//     // selectYears: true,
//     // selectMonths: true,
//     editable: true,
...
//     edit_schedule_date.pickadate('picker').open();
//     event.stopPropagation();
// }

// Latest Working code for piackadate works with : chrome and firefox
var input = $('.edit-ctest-schedule').pickadate({
    editable: true, 
    format: 'yyyy-mm-dd',
    min:true, 
    container: '#edit-ctest-date-picker',
    onClose: function() {
